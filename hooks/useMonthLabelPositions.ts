...
import type { RailLayoutState } from './useMonthLabels';

export function useMonthLabelPositions(dates: BookingDate[], layout: RailLayoutState | null) {
    if (!layout) {
        return {
            primaryIndex: 0,
            secondaryIndex: null,
    ...
    const primaryMarkerIdx = markers.findIndex((m) => m.index === primaryIndex);
